...
class Star(pygame.sprite.Sprite):                               # 创建精灵类Star，继承自pygame.sprite.Sprite
    ...
    def update(self, *args):
        ...
        count = round(pygame.time.get_ticks()/1000, 2)
        word_text(str(count))
...
screen.fill((0,0,0))
font = pygame.font.Font("./font//SourceHanSansSC-Normal.otf", 40)   # 加载字体文件
...
while True:
    ...
    if cyberpi.get_pitch() < -30:              # 利用童芯派陀螺仪俯仰角控制角色上下移动
        y -= 8
    elif cyberpi.get_pitch() > 30:
        y += 8
    elif cyberpi.get_yaw() < -30:              # 利用童芯派陀螺仪偏航角控制角色左右移动
        x -= 8
    elif cyberpi.get_yaw()> 30:
        x += 8
    screen.fill((0,0,0))
